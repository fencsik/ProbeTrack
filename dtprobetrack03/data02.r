### data02 compute d' from data01, and aggregate it by subject, condition,
### gap duration, number of targets, and probe delay
...
f.data02 <- function () {
    ...
    ta <- data01[targ == "distractor", index]
    tp <- data01[targ == "target", index]

    ## create new data table with appropriate IVs and check that everything is
    ## ordered correctly
    if (!all(tp$sub == ta$sub & tp$gapdur == ta$gapdur &
             tp$cond == ta$cond & tp$ntargets == ta$ntargets &
             tp$soa == ta$soa)) {
        stop("mismatch between target-present and target-absent independent variables")
    }
    data02 <- tp[,c("sub", "cond", "gapdur", "ntargets", "soa")]
    ...
    adjHR <- data02$nHit
    index <- adjFA == 0
    if (any(index)) adjFA[index] <- .5
    index <- adjFA == data02$nTA
    if (any(index)) adjFA[index] <- data02$nTA[index] - .5
    index <- adjHR == 0
    if (any(index)) adjHR[index] <- .5
    index <- adjHR == data02$nTP
    if (any(index)) adjHR[index] <- data02$nTP[index] - .5
    ...
    data02$sub <- factor(data02$sub)
    data02$gapdur <- factor(data02$gapdur)
    data02$cond <- factor(data02$cond)
