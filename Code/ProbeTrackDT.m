    ...
    try
        ...
        [subject, blockType, labelType, pointsFlag] = ...
            DialogBox(sprintf('%s Parameters', experiment), ...
                      ...
                      'Block label (1=single,2=shadow,3=gen):', '0', 1, ...
                      'Display points:', '1', 1);
        ...
        blockMesg = sprintf('%s (%s)', blockMesg, briefBlockLabel);
        ...
        for subBlock = subBlockList
            ...
            for trial = 1:nTrials
                ...
                if dataFile == -1
                    header = ['exp\tsub\tcode\trev\tcomp\truntime\t' ...
                              'trialtime\tstdt\tnstim\trefreshdur\t' ...
                              'cuedur\tgapdur\tgapOnsetRange\t' ...
                              'durPostProbe\tgapOnsetTime\tblocktype\t' ...
                              'prac\ttrial\tntargets\tsoa\t' ...
                              'probeTarget\tresp\tacc\trt\tpts\ttpts\t' ...
                              'meanFrameDur\tminFrameDur\tmaxFrameDur\n'];
                else
                    fclose(dataFile);
                    header = [];
                end
                ...
                fprintf(dataFile, ['%s\t%d\t%s\t%s\t%s\t%s\t%s\t%s\t%d\t' ...
                                   '%0.6f\t%d\t%d\t%s\t%d\t%d\t%s\t%d\t' ...
                                   '%d\t%d\t%d\t%d\t%s\t%d\t%d\t%d\t%d\t' ...
                                   '%0.3f\t%0.3f\t%0.3f\n'], ...
                        experiment, subject, mfilename, revision, computer, ...
                        blocktime, trialtime, blockLabel, nStim, ...
                        refreshDuration, durCue, durGap(trial), ...
                        gapOnsetRangeStr, durPostProbe, gapOnsetTime, ...
                        blockTypeStr, prac, trialCounter, nTargets, ...
                        SOA(trial), probeTarget(trial), respString, acc, ...
                        RT, pointsTrial, points, mean(durFrames), ...
                        min(durFrames), max(durFrames(2:end)));
                fclose(dataFile);
