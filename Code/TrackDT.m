    ...
    try
        ...
        for subBlock = subBlockList
            ...
            for trial = 1:nTrials
                ...
                if dataFile == -1
                    header = ['exp\tsub\tcode\trev\tcomp\truntime\ttrialtime\tstdt\t' ...
                ...
                fprintf(dataFile, ['%s\t%d\t%s\t%s\t%s\t%s\t%s\t%s\t'...
                                   '%d\t%0.6f\t%d\t%d\t%s\t%d\t%d\t' ...
                                   '%s\t%d\t%d\t%d\t%d\t%d\t' ...
                                   '%s\t%d\t%d\t%d\t%d\t%0.3f\t%0.3f\t%0.3f\n'], ...
                        experiment, subject, mfilename, revision, computer, blocktime, trialtime, blockLabel, ...
                        nStim, refreshDuration, durCue, durGap(trial), gapOnsetRangeStr, durPostProbe, gapOnsetTime, ...
                        blockTypeStr, prac, trialCounter, nTargets, SOA(trial), probeTarget(trial), ...
                        respString, acc, RT, pointsTrial, points, ...
                        mean(durFrames(2:end)), min(durFrames(2:end)), max(durFrames(2:end)));
                fclose(dataFile);
