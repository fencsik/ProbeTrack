...
%%%
%%% $Id: train.m,v 1.6 2004/05/11 14:19:26 fencsik Exp $
...
% now decode answer
[subject, tracknumber, practicePathsFile, experimentalPathsFile, randomizeTrials] = deal(answer{:});
...
fprintf(dataFile,'experiment,sub,time,pracfile,pathfile,blocktype,shift,trialdur,blankdur,trial,line,speed,ndisks,ntargets,framedur,error,nerrors\n');
fclose(dataFile);
...
%if condition == 1
   ...
   taskInstructionString{7} = 'they will continue moving, and then reappear and stop moving.';
% else
...
responseInstructionString = {
      'When the disks reappear, the arrow cursor will appear.';
...
for block = 1:2
   ...
   for trial = trials
      ...
      % save data
      dataFile = fopen(dataFileName, 'a');
      %header = experiment,sub,time,pracfile,pathfile,blocktype,shift,trialdur,blankdur,trial,line,speed,ndisks,ntargets,framedur,error,nerrors
