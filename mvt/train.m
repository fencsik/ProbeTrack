...
%%% train:
%%% presents training trials in the Multiple Object Tracking 
%%% experiments. Uses path files generated by generator.m
%%% Authors: David Fencsik (based on file by Todd Horowitz)
%%%
%%% $Id: train.m,v 1.13 2004/08/20 14:08:29 fencsik Exp $
...
ExperimentName = 'ShiftTrack6Train';
minimumAccuracy = '60%';
...
% now decode answer
[subject, tracknumber, experimentalPathsFile, set10BitDAC, randomizeTrials] = deal(answer{:});
...
fprintf(dataFile,'experiment,sub,time,pathfile,blocktype,movetype,trialdur,blankdur,trial,line,speed,ndisks,ntargets,framedur,error,nerrors\n');
fclose(dataFile);
...
taskInstructionString{4} = ['After the target' nounEnd ' stop' verbEnd ' blinking, all the disks will start to move around the screen.'];
taskInstructionString{5} = '';

if tracknumber == 1
   taskInstructionString{6} = 'Your task is to keep track of the target throughout the trial.';
else
   taskInstructionString{6} = ['Your task is to keep track of the ' tracknumberString ' targets throughout the trial.'];
end;
...
for block = 1
   ...
   for trial = trials
      ...
      % save data
      dataFile = fopen(dataFileName, 'a');
      %header = experiment,sub,time,pathfile,blocktype,movetype,trialdur,blankdur,trial,line,speed,ndisks,ntargets,framedur,error,nerrors\n');
      count = fprintf(dataFile, ...
                      '%s,%s,%s,%s,%s,%d,%d,%d,%d,%d,%d,%d,%d,%6.4f,%d,%d\n', ...
                      ExperimentName, subject, timestamp, experimentalPathsFile, blocktype, ...
...
if trialcounter > 0
   ['Average number tracked = ' num2str(sum(nTracked(1:trialcounter)) / trialcounter, 3) ' out of ' num2str(tracknumber) ' targets.']
end;
