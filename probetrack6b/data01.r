### data01: aggregate accuracy and RT statistics by subject, gap duration, probe
### type, number of targets, and probe delay
###
### $LastChangedDate$
...
### extract factors for all trials and for all correct trials
    ...
    factorsA <- with(data00, list(soa=soa, target=target, ntargets=ntargets, gapdur=gapdur, sub=sub));
    factorsC <- with(dataCC, list(soa=soa, target=target, ntargets=ntargets, gapdur=gapdur, sub=sub));
