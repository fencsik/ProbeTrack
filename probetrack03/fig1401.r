### fig1401.r: plot fit of model against observed data, averaged across
### subjects
...
do.fig1401 <- function () {
   ...
   dt <- with(data14$data, tapply(rt, list(soa), mean))
   subjects <- rownames(dt)

   ## gather parameters for model fits
   rtime <- with(data14$fit, tapply(rtime, list(subjects), mean))
   baseRT <- with(data14$fit, tapply(baseRT, list(subjects), mean))

   ## fit model to each subject in each condition
   Subjects <- dimnames(rtime)[[1]]
   predx <- seq(0, 1480, by=1)
   ...
   pdf(outfile, width=6, height=6, pointsize=14)
