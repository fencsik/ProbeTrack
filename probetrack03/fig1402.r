### fig1402.r: plot fit of model against observed data for each subject
...
do.fig1402 <- function () {
   ...
   model <- data14$model
   fit <- data14$fit
   dt <- with(data14$data, tapply(rt, list(soa, sub), mean))

   x <- as.numeric(dimnames(dt)[[1]])
   subList <- dimnames(dt)[[2]]

   pdf(outfile, width=18, height=8, pointsize=14)
   opar <- par(mfrow=c(2, 4), las=1, pty="m", cex.axis=.6,
               xpd=NA, bg="white")

##    ## set up color matrix
##    col <- as.character(with(data14$data, tapply(rt, list(gapdur, ntargets), mean)))
