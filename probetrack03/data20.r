...
do.data20 <- function () {
   ...
   dt <- read.csv(infile)
   dt$acc <- 1 - dt$error # switch from error to acc
   dt <- dt[dt$sub != "tjo", ] # remove subject with high error rate
   ...
   print(with(dt[dt$badkey == 1 | is.na(dt$rt), ],
              tapply(trial, list(sub), length)))
