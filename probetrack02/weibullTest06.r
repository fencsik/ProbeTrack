...
plotfit <- function(file, data, fit) {
   ...
   plot(x, dt, type = "n", bty = "n",
        axes = F,
        ##ylim = c(min(fit[, "baseline"], data[, "rt"]), max(fit[, "asymptote"])),
        ylim = c(700, 900),
...
do.weibullTest06 <- function () {
   ...
   ## optionally remove ssf and el because of low accuracy
   ##data03 <- data03[data03$sub != "ssf" & data03$sub != "el", ];

   ## set-up data matrix
   data03$sub <- as.character(data03$sub); print(sort(unique(data03$sub)));
