### weibullTest04.r: tests fitting of weibull to each subject's data with various
### starting parameters
###
### $LastChangedDate$
...
do.weibullTest04 <- function () {
   thisfile <- "weibullTest04.r";
   infile <- "data03.rda";
...
###    }
   ...
   print(p0);
   cat("End parameters:\n");
   print(out <- fit[, c(1:6, 11:13)]);
   print(apply(out[,-(1:3)], 2, mean));
   plotfit(outfile, data, fit);
   par(pty="s");
   index <- order(fit[, "r.sq"]);
   x11();
   plot(fit[index, "r.sq"], type = "n", pch = 21, ylim = c(0, 1), main=paste(p0[1:2], sep="-"));
   text(fit[index, "r.sq"], labels = fit[index, "sub"]);
}

print(system.time(do.weibullTest04(), TRUE));
rm(do.weibullTest04);
